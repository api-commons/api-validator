...
The latest-draft JSON Schema for every artifact type the [API Validator](../) lints — one file per artifact, named `<artifact-id>.schema.json`. Bare names are the **latest** version of each spec; `-<version>` files are still-active alternate versions.
