...
// Curated experience/spec tags for the upstream spectral:* built-in rules.
...
// Spectral ships built-in rulesets for OpenAPI + AsyncAPI only; Arazzo and JSON
// Schema are covered entirely by the curated inline rules (no `extends`).
...
// ---- search source selector (GitHub default; GitLab/Bitbucket opt-in) --------
const sourceSelect = $<HTMLSelectElement>('#source-select');
let currentSource: SourceId = 'github';
export function populateSources() {
  ...
  if (!enabled.some((s) => s.id === currentSource)) currentSource = enabled[0]?.id ?? 'github';
...
const FILTER_KEY = 'api-validator:filter';
...
async function runLint() {
  ...
  const model = docEditor.getModel()!;
  if (error) {
    monaco.editor.setModelMarkers(model, 'api-validator', []);
...
const CFG_SECRETS = ['cfg-github', 'cfg-gitlab', 'cfg-bitbucket']; // password fields
(function initConfig() {
  const cfg = loadConfig();
...
$('#download-apisjson').addEventListener('click', () => {
  ...
  const yaml = buildApisJson('API Validator artifacts', docs.map((d) => ({ name: d.name, content: d.content, lang: d.lang, type: d.type })));
...
initEngage(() => {
  ...
  return 'Context from the API Validator:\n- ' + parts.join('\n- ');
});
...
async function gitSaveDoc(id: string, kind: 'commit' | 'pr') {
  ...
  const message = `${kind === 'pr' ? 'Propose' : 'Update'} ${path} via api-validator`;
