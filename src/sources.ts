// Multi-source code search: GitHub (default) + GitLab/Bitbucket code search.
// Sources are toggled in Config. Token-arg based (no Config coupling).
export type SourceId = 'github' | 'gitlab' | 'bitbucket';
export interface SourceDef { id: SourceId; label: string; on: boolean; }
// GitHub on by default; GitLab + Bitbucket are opt-in.
...
export interface Hit { source: SourceId; name: string; repo?: string; path?: string; ref?: string; url?: string }

// GitHub code-search qualifier per artifact id (appended to the user's query).
const GH_QUALIFIER: Record<string, string> = {
  openapi: 'openapi extension:yaml', asyncapi: 'asyncapi extension:yaml',
  arazzo: 'arazzo extension:yaml', 'json-schema': '"$schema" extension:json',
};
const b64decode = (s: string) => decodeURIComponent(escape(atob(s.replace(/\s/g, ''))));

export interface ArtifactRef { id: string }
