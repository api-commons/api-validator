...
const SERVICES: Service[] = [
  ...
  },
  {
    ...
    blurb: 'We encode your organization’s standards as portable Spectral rules you can run in CI, the editor, and the browser.',
    cta: 'Talk rulesets',
...
function mailto(s: Service, ctx: string): string {
  const body = `${s.body(ctx)}\n\n— sent from ${APP} (validator.apicommons.org)`;
  ...
  modal.innerHTML = `
    <div class="modal-card engage-card">
      ...
      <div class="engage-body">
        <p class="engage-intro">API Validator is open and free to run yourself. When you want experts in the loop,
